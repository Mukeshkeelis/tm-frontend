function collapseItem(theme, ownerState) {
  ...
  return {
    background: active ? dark.main : transparent.main,
...
function collapseIconBox(theme, ownerState) {
  ...
  const { white, info, light, text, gradients } = palette;
  const { md } = boxShadows;
  const { borderRadius } = borders;
  const { pxToRem } = functions;
  ...
  return {
    ...
    "& svg, svg g": {
      fill: active ? gradients.dark.main : gradients.dark.state,
    },
  };
}
...
function collapseArrow(theme, ownerState) {
  ...
  const { black, dark, gradients } = palette;
  ...
  return {
    ...
    color: open ? black.main : black.main,
